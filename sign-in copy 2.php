...
// Gerekli dosyaları ve autoloader'ı çağır
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submitForm'])) {
    ...
    $errors = $authController->handleLoginRequest();
}
        ...
        <div class="auth-cover-sidebar-inner">
            <div class="auth-cover-card-wrapper">
                <div class="auth-cover-card p-sm-5">
                    ...
                    <?php
                    // --- YENİ EKLENEN MESAJ GÖSTERME ALANI ---

                    // URL'den gelen durum mesajlarını yönet
                    if (isset($_GET['status'])) {
                        $message = '';
                        $messageType = 'info'; // Varsayılan mesaj türü (bilgi)

                        switch ($_GET['status']) {
                            case 'demo_expired':
                                $message = '<strong>Demo Süreniz Doldu!</strong><br>Sistemi kullanmaya devam etmek için lütfen bizimle iletişime geçin.';
                                $messageType = 'warning'; // Uyarı türü
                                break;
                            
                            case 'logged_out':
                                $message = 'Başarıyla çıkış yaptınız.';
                                $messageType = 'success'; // Başarı türü
                                break;

                            case 'session_expired':
                                $message = 'Oturumunuz zaman aşımına uğradı. Lütfen tekrar giriş yapın.';
                                $messageType = 'info'; // Bilgi türü
                                break;
                        }
                        
                        // Mesajı Bootstrap alert class'ları ile formatla
                        if (!empty($message)) {
                            echo '<div class="alert alert-' . $messageType . ' text-center">' . $message . '</div>';
                            
                        }
                    }

                    // Form gönderiminden kaynaklanan hataları göster
                    if (!empty($errors)) {
