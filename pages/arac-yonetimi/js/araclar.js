let aracEkleUrl = "/pages/arac-yonetimi/api/CarApi.php";
...
$(document).on("click", "#AracEkle", function () {
  ...
  $("#modalAracPlaka").on("input", function () { this.value = this.value.toUpperCase(); });
  ...
  var validator = $("#aracEkleForm").validate({
    rules: {
      ...
      modalAracPlaka: { required: true, plakaKontrol: true },
    ...
    messages: {
      ...
      modalAracPlaka: { required: "Lütfen araç plakası giriniz", plakaKontrol: "Geçerli bir plaka giriniz (örn: 34 ABC 123)" }
    }
  });
  ...
  fetch(aracEkleUrl, { method: "POST", body: formData })
    ...
    .then((data) => {
      ...
      Swal.fire({ title: data.status === "success" ? "Başarılı" : "Hata", text: data.message, icon: data.status, confirmButtonText: "Tamam" });
    })
    .catch((error) => { console.error("Kayıt sırasında hata:", error); Swal.fire("Hata", "Kayıt sırasında bir hata oluştu.", "error"); });
});
...
$(document).on("click", ".delete-car", function () {
  ...
  Swal.fire({ title: "Emin misiniz?", html: `${carName} <br> plakalı aracı silmek istediğinize emin misiniz?`, icon: "warning", showCancelButton: true, confirmButtonText: "Evet", cancelButtonText: "Hayır", }).then((result) => {
...
function updateRowNumbers() {
  $("#aracList tbody tr").each(function (index) { $(this).find(".sira-no").text(index + 1); });
}