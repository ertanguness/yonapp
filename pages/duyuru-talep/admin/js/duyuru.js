function duyuruInit() {
    ...
    const dt = $tbl.DataTable({
      ...
      columns: [
        ...
        { data: 5 },
        {
          data: 6,
          render: function (id) {
            const editUrl = "duyuru-duzenle/" + id;
            return (
              '<a href="' +
              editUrl +
          ...
          fetch(apiUrl, {
            ...
            .then(function (data) {
              swal
                ...
                .then(function (res) {
                  if (res.isConfirmed && data.status === "success") {
  ...
  if ($form.length) {
    ...
    if (!quill) {
      $("#contentEditor, #content").on("input", updatePreview);
    } else {
    ...
    function toggleTargets() {
      ...
      try {
        $(".select2").select2({ dropdownParent: $form });
      } catch (e) {}
      updatePreview();
    }
    ...
    $form.on("submit", function (e) {
      ...
      if (($targetType.val() || "") === "block" && ($blockId.val() || "")) {
        fd.append("block_id", $blockId.val());
      }
      if (($targetType.val() || "") === "kisi") {
        var ids = $kisiIds.val() || [];
        if (ids.length) {
          fd.append("kisi_ids", ids.join(","));
        }
      }
      if (id) {
        fd.append("id", id);
