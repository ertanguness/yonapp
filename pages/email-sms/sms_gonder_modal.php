...
<style>
    ...
    .modal-dots .dot.dot-r {
        background: #ff6b6b
    }

    .modal-dots .dot.dot-y {
        background: #feca57
    }

    .modal-dots .dot.dot-g {
        background: #1dd1a1
    }
    ...
    .template-actions.compact .btn .label {
        display: none
    }
...
<div class="sms-sender-card shadow-lg" data-kisi-telefon="<?php echo htmlspecialchars($telefonNumarasi ?? 0, ENT_QUOTES); ?>" style="max-height:85vh; overflow:auto;">
    ...
    <div class="card-body p-4">
        <div class="row g-5">
            ...
            <?php if ($SettingsHelper->getMessageSubjectsCount() == 0): ?>
                <div class="alert alert-warning d-flex align-items-center" role="alert">
                    <i class="fas fa-exclamation-triangle me-2"></i>
                    <span>Gönderen adı (başlık) tanımlı değil. Lütfen <a href="/settings/sms" class="alert-link">SMS ayarları</a> sayfasından gönderen adınızı ekleyin.</span>
                </div>
            <?php else: ?>
...
<script>
        ...
        if (el && !el.dataset.bound) {
            ...
            el.addEventListener('change', function() {
                ...
                if (this.checked && !has) {
                    ...
                    ta.value = val.slice(prefix.length);
                }
                ta.dispatchEvent(new Event('input'));
            });
        }
    ...
    (function() {
        ...
        if (tplModalEl && tplModalEl.parentNode !== document.body) {
            document.body.appendChild(tplModalEl);
        }
        if (saveModalEl && saveModalEl.parentNode !== document.body) {
            document.body.appendChild(saveModalEl);
        }
        if (tplModalEl) {
            tplModalEl.setAttribute('data-bs-backdrop', 'static');
        }
        if (saveModalEl) {
            saveModalEl.setAttribute('data-bs-backdrop', 'static');
        }
        ...
        function renderSkeleton() {
            ...
            for (var i = 0; i < 5; i++) {
                ...
                left.className = 'd-flex align-items-center w-100';
                var av = document.createElement('div');
                av.className = 'skeleton-avatar me-3';
                var wrap = document.createElement('div');
                wrap.className = 'w-100';
                var l1 = document.createElement('div');
                l1.className = 'skeleton-line mb-2';
                l1.style.width = '40%';
        ...
        function renderTemplates(items) {
            ...
            items.filter(function(it) {
                ...
                avatar.textContent = (it.name || '?').charAt(0).toUpperCase();
                ...
                subtitle.textContent = (it.body || '').replace(/\s+/g, ' ').slice(0, 100);
                ...
                applyBtn.className = 'avatar-text avatar-sm';
                applyBtn.setAttribute('data-bs-toggle', 'tooltip');
                applyBtn.setAttribute('data-bs-trigger', 'hover');
                applyBtn.setAttribute('title', 'Uygula');
                applyBtn.setAttribute('aria-label', 'Uygula');
                applyBtn.innerHTML = '<i class="feather feather-check fs-12"></i>';
                ...
                editBtn.className = 'avatar-text avatar-sm';
                editBtn.setAttribute('data-bs-toggle', 'tooltip');
                editBtn.setAttribute('data-bs-trigger', 'hover');
                editBtn.setAttribute('title', 'Düzenle');
                editBtn.setAttribute('aria-label', 'Düzenle');
                editBtn.innerHTML = '<i class="feather feather-eye fs-12"></i>';
                ...
                delBtn.className = 'avatar-text avatar-sm';
                delBtn.setAttribute('data-bs-toggle', 'tooltip');
                delBtn.setAttribute('data-bs-trigger', 'hover');
                delBtn.setAttribute('title', 'Sil');
                delBtn.setAttribute('aria-label', 'Sil');
                delBtn.innerHTML = '<i class="feather feather-more-vertical"></i>';
                delBtn.addEventListener('click', function() {
                    if (!confirm('Şablon silinsin mi?')) return;
                    fetch('/pages/email-sms/api/templates.php?id=' + encodeURIComponent(it.id) + '&csrf_token=' + encodeURIComponent(window.csrfToken || ''), {
                            method: 'DELETE'
                        })
                        .then(function(r) {
                            return r.json();
        ...
        confirmSaveBtn?.addEventListener('click', function() {
            ...
            if (isEdit) {
                payload.id = parseInt(this.dataset.editId, 10) || 0;
            }
            fetch(url, {
                    method: method,
                    headers: {
