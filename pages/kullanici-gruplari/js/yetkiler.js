document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(url, {
    ...
    .then((data) => {
      if (data && data.data) {
        ...
        userPermissions = (data.data.user_permissions || []).map((id) =>
          parseInt(id, 10)
        );
  ...
  function simulateLoading(isLoading = true) {
    ...
    if (permissionContainer)
      permissionContainer.style.display = isLoading ? "none" : "block";
    if (!isLoading && permissionGroups.length === 0) {
      // Eğer veri yüklenememişse bir mesaj gösterilebilir
      if (permissionContainer)
        permissionContainer.innerHTML =
          "<p class='text-center text-muted'>Yetki verileri yüklenemedi.</p>";
    }
  }
  ...
  function renderViews() {
    ...
    const currentSearchTerm = permissionSearchInput
      ? permissionSearchInput.value.toLowerCase().trim()
      : "";
  ...
  function renderCardView() {
    ...
    permissionGroups.forEach((group) => {
      ...
      const isGroupInitiallyExpanded =
        selectedInGroup > 0 ||
        (searchTerm &&
      ...
      <div class="permission-group mb-3" data-group-name="${group.name.toLowerCase()}" data-group-id="${group.id
          ...
          <div class="group-header d-flex justify-content-between align-items-center ${selectedInGroup > 0 ? "active" : ""
            ...
            <div class="d-flex align-items-center flex-grow-1">
              <div class="permission-icon me-3"><i class="feather-${group.icon || "shield"
        }"></i></div>
              <div>
            ...
            <div class="d-flex align-items-center">
              <span class="badge bg-opacity-10 fw-bold text-muted rounded-pill badge-count me-3 group-selected-count">${selectedInGroup}/${totalInGroup}</span>
              <i class="ti ti-chevron-right arrow-icon ${isGroupInitiallyExpanded ? "rotated" : ""
        }"></i>
            </div>
          </div>
          ...
          .map(
            (perm) => `
              <div class="permission-item ${userPermissions.includes(perm.id) ? "selected" : ""
              }" data-id="${perm.id
              }" data-perm-name="${perm.name.toLowerCase()}">
                <div class="flex-grow-1">
                  <h6 class="mb-0 fw-semibold perm-name-text">${perm.name}</h6>
                  ${perm.required
                ? '<span class="badge bg-danger-subtle text-danger-emphasis rounded-pill ms-1">Zorunlu</span>'
                : ""
              }
                  <small class="text-muted d-block perm-description-text">${perm.description || ""
              }</small>
                </div>
                <div class="form-check form-switch ms-3">
                  <input class="form-check-input permission-checkbox" type="checkbox" data-id="${perm.id
              }" ${userPermissions.includes(perm.id) ? "checked" : ""} ${perm.required ? "disabled" : ""
              }>
                </div>
              </div>`
          )
  ...
  function renderTreeView() {
    ...
    permissionGroups.forEach((group) => {
      const isGroupInitiallyExpanded =
        group.permissions.some((p) => userPermissions.includes(p.id)) ||
        (searchTerm &&
          (group.name.toLowerCase().includes(searchTerm) ||
            group.permissions.some((p) =>
              p.name.toLowerCase().includes(searchTerm)
      ...
      html += `
        <li>
          <div class="tree-node">
            <a class="tree-toggle ${isGroupInitiallyExpanded ? "expanded" : "collapsed"
          ...
          .map(
            (perm) => `
              <li>
                <div class="permission-item" data-id="${perm.id
              }" data-perm-name="${perm.name.toLowerCase()}">
                  <div class="flex-grow-1">
  ...
  function renderFilterChips() {
    ...
    const uniqueGroupNames = [
      ...new Set(permissionGroups.map((g) => g.name.toLowerCase()))
    ];
    ...
    uniqueGroupNames.forEach((groupName) => {
      ...
      const originalGroupName =
        permissionGroups.find((g) => g.name.toLowerCase() === groupName)
          ?.name || groupName;
  ...
  function applyCardViewFilters() {
    ...
    const groupsElements =
      cardViewContainer.querySelectorAll(".permission-group");

    groupsElements.forEach((groupEl) => {
      const groupName = groupEl.dataset.groupName;
      const groupData = permissionGroups.find(
    ...
    groupLiElements.forEach((liEl) => {
      ...
      if (subList) {
        ...
        if (isVisibleByChip && isVisibleBySearch) {
          ...
          if (searchTerm && hasVisiblePermsInSearchInTree) {
            // Arama sonucu varsa
            shouldExpand = true;
          }
          ...
          if (isSelectAllActive && selectedGroups.length === 0) {
            // Tümünü seç ve filtre yoksa
            shouldExpand = true;
          }
        }
  ...
  function updateSelectedCountUI() {
    ...
    document
      ...
      .forEach((groupEl) => {
        ...
        if (groupData) {
          const selectedInGroup = groupData.permissions.filter((p) =>
            selectedIds.has(p.id)
          ).length;
          const countDisplay = groupEl.querySelector(".group-selected-count");
          if (countDisplay)
            countDisplay.textContent = `${selectedInGroup}/${groupData.permissions.length}`;
  ...
  function handlePermissionSelection(checkbox) {
    ...
    document
      .querySelectorAll(`.permission-checkbox[data-id="${permId}"]`)
      .forEach((cb) => {
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        Swal.fire(
          ...
          "Yetkiler orijinal haline geri alındı.",
          "info"
        );
      });
  }
  ...
  $(document).on("click", "#selectAllPermissions", function () {
    ...
    Toastify({
      text: "Tüm yetkiler başarıyla seçildi.",
      duration: 3000,
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        fetch(url, {
          method: "POST",
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
          body: `action=savePermissions&id=${encodeURIComponent(
            roleID
  ...
  if (document.getElementById("selectHighlighted")) {
    document
      .getElementById("selectHighlighted")
      .addEventListener("click", function () {
        if (!searchTerm) {
        ...
        const currentViewCheckboxes = showTreeViewCheckbox.checked
          ? treeViewContainer.querySelectorAll(
            ".permission-checkbox:not(:disabled)"
          )
          : cardViewContainer.querySelectorAll(
            ".permission-checkbox:not(:disabled)"
          );
        ...
        currentViewCheckboxes.forEach((checkbox) => {
          ...
          if (
            ...
            // offsetParent null değilse görünürdür
        ...
        Swal.fire(
          selectedAny ? "Başarılı" : "Uyarı",
          selectedAny
            ? "Arama sonucuyla eşleşen yetkiler seçildi."
            : "Eşleşen yeni yetki bulunamadı veya zaten seçili.",
